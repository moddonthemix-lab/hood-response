...
| **Sniper (auto-buy)** | optional **Sniper** dashboard tab that auto-buys qualifying alerts with a server **hot wallet** (ethers). Settings: buy-conviction band (default 60–100), buy amount (0.0005 ETH minimum), and a **take-profit %** that auto-sells a position when it hits target. Shows live **positions**, current value, and **PnL** (open + realized). **OFF by default**; real swaps only run once a dedicated burner wallet + verified DEX router are configured. Rails: per-trade cap, daily spend cap, and an on/off kill switch. Admin-gated. See `SNIPER_*` |
